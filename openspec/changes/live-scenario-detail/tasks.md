...
- [ ] 5.3 Manually verify against a running assessment: a `matching` scenario shows executor/IDs and a live "k/n matched" assertion bar that fills in before completion, and the final state matches today's completed view. (Requires a live deployment with an Elastic target — must be verified by the user.)
