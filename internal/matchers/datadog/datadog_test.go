...
func TestDatadog(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			mockDatadog := &mocks.MockDatadogSecuritySignalsAPI{}
			...
			matches, err := matcher.HasExpectedAlert([]string{detonationUid}, nil)
			require.Nil(t, err)
			...
			// CLEANUP
			//TODO split test?
			err = matcher.Cleanup([]string{detonationUid}, nil)
			require.Nil(t, err)
