<script lang="ts">
	...
	import NewAssessmentTypeDialog from '$lib/components/NewAssessmentTypeDialog.svelte';
	...
	import type { Assessment, Schedule, ScenarioType } from '$lib/types';
	...
	let pageAssessments = $state<Assessment[]>([]);
	let total = $state(0);
	let page = $state(1);
	...
	let deleteTarget = $state<Assessment | null>(null);
	let deleting = $state(false);

	let scheduleDialogOpen = $state(false);
	let scheduleTarget = $state<Assessment | null>(null);
	...
	async function load() {
		...
		const data = await loadAssessmentPage(page, perPage, currentFilters());
		if (seq !== requestSeq) return;
		pageAssessments = data.assessments;
		total = data.total;
	}
	...
	function openDelete(e: Event, assessment: Assessment) {
		e.stopPropagation();
		deleteTarget = assessment;
	...
	async function handleDelete() {
		...
		try {
			...
			if (pageAssessments.length === 1 && page > 1) {
				page--;
				syncUrl();
			}
			await Promise.all([load(), loadScheduleMap()]);
	...
	function openSchedule(e: Event, assessment: Assessment) {
		e.stopPropagation();
		scheduleTarget = assessment;
		scheduleDialogOpen = true;
	}

	async function startRename(e: Event, assessment: Assessment) {
		e.stopPropagation();
		renameId = assessment.id;
		renameValue = assessment.name;
		await tick();
		renameInputEl?.focus();
	...
	async function commitRename(assessment: Assessment) {
		if (renameSaving) return;
		const next = renameValue.trim();
		if (!next || next === assessment.name) {
			cancelRename();
			return;
		}
		renameSaving = true;
		error = '';
		try {
			await updateAssessment(assessment.id, next, assessment.yaml, assessment.type);
			await load();
			renameId = null;
			renameValue = '';
		} catch (e) {
	...
	function handleRenameKeydown(e: KeyboardEvent, assessment: Assessment) {
		if (e.key === 'Enter') {
			e.preventDefault();
			commitRename(assessment);
...
<div class="space-y-6">
	...
	{:else if pageAssessments.length === 0}
		<Empty.Root>
			<Empty.Header>
				...
				<Empty.Title>
					{hasActiveFilters ? 'No matching assessments' : 'No saved assessments'}
				</Empty.Title>
				<Empty.Description>
					{hasActiveFilters
						? 'Try clearing or adjusting your filters.'
						: 'Save assessment YAML files here for quick access.'}
				</Empty.Description>
			</Empty.Header>
			<Empty.Content>
				{#if hasActiveFilters}
				...
				{:else}
					<Button onclick={() => (newDialogOpen = true)}>New Assessment</Button>
				{/if}
			</Empty.Content>
		</Empty.Root>
	{:else}
		<div class="animate-fade-up stagger-2 overflow-hidden rounded-lg border">
			<Table.Root>
				...
				<Table.Body>
					{#each pageAssessments as assessment}
						<Table.Row
							class="group cursor-pointer hover:bg-accent/50 transition-colors"
							onclick={() => {
								if (renameId !== assessment.id) goto(`/assessments/${assessment.id}`);
							}}
						>
							<Table.Cell class="font-medium">
								{#if renameId === assessment.id}
									<div
									...
									>
										<Input
											...
											onkeydown={(e: KeyboardEvent) => handleRenameKeydown(e, assessment)}
											onblur={() => commitRename(assessment)}
								...
								{:else}
									<div class="flex items-center gap-1.5">
										<span class="truncate">{assessment.name}</span>
										<button
											type="button"
											title="Rename"
											...
											onclick={(e) => startRename(e, assessment)}
										>
							...
							<Table.Cell>
								{#if scheduleMap.has(assessment.id)}
									{@const schedule = scheduleMap.get(assessment.id)}
									...
									{#if assessment.createdBy && assessment.createdBy !== 'anonymous'}
										<Tooltip.Root>
											<Tooltip.Trigger class="text-xs text-muted-foreground cursor-default w-fit">
												by {formatUserEmail(assessment.createdBy)}
											</Tooltip.Trigger>
							...
							<Table.Cell>
								<div class="flex flex-col">
									...
									{#if assessment.updatedBy && assessment.updatedBy !== 'anonymous'}
										<Tooltip.Root>
											<Tooltip.Trigger class="text-xs text-muted-foreground cursor-default w-fit">
												by {formatUserEmail(assessment.updatedBy)}
											</Tooltip.Trigger>
											<Tooltip.Content>{assessment.updatedBy}</Tooltip.Content>
										</Tooltip.Root>
									{/if}
								</div>
							</Table.Cell>
							<Table.Cell>
								<div class="flex justify-end gap-1">
									<Button
										...
										onclick={(e: Event) => openSchedule(e, assessment)}
									>
									...
									<Button
										...
										onclick={(e: Event) => openDelete(e, assessment)}
									>
...
<!-- New Assessment type picker -->
<NewAssessmentTypeDialog bind:open={newDialogOpen} />
