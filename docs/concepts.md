...
**Elastic Security alert matcher** — polls Kibana for a Detection Engine alert whose `kibana.alert.rule.name.keyword` matches the expected rule name.

**Datadog security signal matcher** — polls Datadog Security Signals for a signal whose name matches exactly.

Both matchers poll on a configurable interval until the alert appears or the scenario timeout is reached.
...
Every detonation is assigned a UUID at execution time. Where possible, SimRun reflects that UUID into the generated activity — for example, as a process argument or user-agent string — so that the resulting alert maps unambiguously to exactly one detonation event.
